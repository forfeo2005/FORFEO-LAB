...
(function() {
    ...
    const config = {
        botName: "Forfy",
        // Assure-toi que la vidéo est bien à cet endroit (racine du site)
    ...
    const knowledgeBase = {
        ...
        "salut": "Salut ! Prêt à explorer ?",
        
        // Tes demandes spécifiques
        ...
        "contact": "Besoin d'aide ? Écrivez-nous à support@forfeolab.com ou visitez notre page contact sur le site principal.",
        "prix": "L'accès est gratuit pour nos ambassadeurs ! Vous profitez d'expériences offertes en échange de vos rapports.",
        "services": "Nous offrons des services d'audit qualité, de clients mystères et de création de contenu pour les établissements.",
        
        "aide": "Je peux répondre à : 'C'est quoi ?', 'Contact', 'Prix' ou 'Services'.",
        "default": "Je ne suis pas sûr de comprendre. Essayez de demander 'Aide', 'Contact' ou 'C'est quoi ?'."
    };
    ...
    const getPageMessage = () => {
        ...
        if (path.includes("dashboard.html")) return "Prêt pour une mission ? 🚀";
        if (path.includes("login.html")) return "Connectez-vous ici 🔐";
        if (path.includes("candidature.html")) return "Rejoignez l'équipe ! ⭐";
    ...
    style.innerHTML = `
        #forfy-widget { position: fixed; bottom: 30px; right: 30px; z-index: 10000; font-family: 'Inter', sans-serif; display: flex; flex-direction: column; align-items: flex-end; }
        #forfy-bubble { background: white; padding: 10px 15px; border-radius: 15px 15px 5px 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.1); margin-bottom: 10px; font-size: 14px; font-weight: 600; color: #1e293b; opacity: 0; transform: translateY(10px); animation: forfyFadeIn 0.5s forwards 1s; border: 1px solid #e2e8f0; max-width: 200px; text-align: right; }
        #forfy-toggle { width: 70px; height: 70px; border-radius: 50%; overflow: hidden; cursor: pointer; box-shadow: 0 8px 25px rgba(37, 99, 235, 0.4); border: 3px solid white; background: black; transition: transform 0.3s; }
        #forfy-toggle:hover { transform: scale(1.1); }
        #forfy-toggle video { width: 100%; height: 100%; object-fit: cover; }
        #forfy-window { position: fixed; bottom: 110px; right: 30px; width: 350px; height: 450px; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); border-radius: 20px; box-shadow: 0 20px 50px rgba(0,0,0,0.15); display: none; flex-direction: column; border: 1px solid rgba(255,255,255,0.5); overflow: hidden; animation: forfySlideUp 0.3s ease; }
        .forfy-header { background: linear-gradient(135deg, ${config.primaryColor}, ${config.secondaryColor}); padding: 15px; color: white; display: flex; justify-content: space-between; align-items: center; }
        ...
        <div id="forfy-widget">
            ...
            <div id="forfy-window">
                ...
                <div class="forfy-input-area">
                    <input type="text" id="forfy-input" placeholder="Posez une question..." onkeypress="handleKeyPress(event)">
    ...
    window.toggleForfy = function() {
        ...
        if (isOpen) {
            ...
            if (!hasGreeted) {
                setTimeout(() => addBotMessage("Bonjour ! Je suis Forfy. Comment puis-je vous aider ?"), 500);
                hasGreeted = true;
            }
